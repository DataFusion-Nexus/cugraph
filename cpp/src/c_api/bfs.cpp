...
// Build a storage-aligned packed-bool edge mask from a set of external edge IDs.
// NOTE: this currently does the matching host-side -- it copies each edge partition's
// edge IDs to host and runs an O(E * log(N)) binary search over the sorted include set.
// Acceptable for small include sets and modest graph sizes, but a future optimization
// could push the lookup onto the device via thrust::binary_search or a hash table.
  ...
  edge_property_t<edge_t, edge_t> const& graph_edge_ids)
{
  ...
  std::vector<edge_t> h_include_ids(include_ids.size());
  raft::update_host(
    h_include_ids.data(), include_ids.data(), include_ids.size(), handle.get_stream());
  handle.sync_stream();
  std::sort(h_include_ids.begin(), h_include_ids.end());
  h_include_ids.erase(std::unique(h_include_ids.begin(), h_include_ids.end()),
                      h_include_ids.end());

  edge_property_t<edge_t, bool> edge_mask(handle, graph_view);
  auto edge_mask_view = edge_mask.mutable_view();
  auto edge_ids_view  = graph_edge_ids.view();

  for (size_t i = 0; i < graph_view.number_of_local_edge_partitions(); ++i) {
    auto const edge_count = static_cast<size_t>(edge_ids_view.edge_counts()[i]);
    auto const num_words  = cugraph::packed_bool_size(edge_count);
    std::vector<edge_t> h_edge_ids(edge_count);
    std::vector<uint32_t> h_edge_mask(num_words, cugraph::packed_bool_empty_mask());
    raft::update_host(
      h_edge_ids.data(), edge_ids_view.value_firsts()[i], edge_count, handle.get_stream());
    handle.sync_stream();

    for (size_t edge_offset = 0; edge_offset < edge_count; ++edge_offset) {
      if (std::binary_search(h_include_ids.begin(), h_include_ids.end(), h_edge_ids[edge_offset])) {
        h_edge_mask[cugraph::packed_bool_offset(edge_offset)] |=
          cugraph::packed_bool_mask(edge_offset);
      }
    }

    raft::update_device(
      edge_mask_view.value_firsts()[i], h_edge_mask.data(), h_edge_mask.size(), handle.get_stream());
  }
