...
def bfs(ResourceHandle handle, _GPUGraph graph,
    ...
    if not all(has_vertex(handle, graph, sources, do_expensive_check)):
        raise ValueError(
            "one or more vertices are invalid. Call the method 'has_vertex' "
...
def bfs_with_predicates(ResourceHandle handle, _GPUGraph graph,
    ...
    """
    Performs a single-GPU BFS with optional concrete predicate sets.

    Predicate inputs must be device arrays. Python callables are not accepted by
    this API; build vertex sets or edge-id sets before calling.
    """
    assert_CAI_type(sources, "sources")
    ...
    try:
        ...
        include_vertices_view_ptr = \
            create_cugraph_type_erased_device_array_view_from_py_obj(include_vertices)
        exclude_vertices_view_ptr = \
            create_cugraph_type_erased_device_array_view_from_py_obj(exclude_vertices)
        target_vertices_view_ptr = \
            create_cugraph_type_erased_device_array_view_from_py_obj(target_vertices)
        include_edge_ids_view_ptr = \
            create_cugraph_type_erased_device_array_view_from_py_obj(include_edge_ids)
        ...
        if predicate_result_ptr != NULL:
            target_info = {
                "target_found": bool(
                    cugraph_bfs_predicate_result_get_target_found(predicate_result_ptr)
                ),
                "target_distance": cugraph_bfs_predicate_result_get_target_distance(
                    predicate_result_ptr
                ),
            }
        else:
