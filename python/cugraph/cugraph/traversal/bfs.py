...
def _prepare_vertex_set(G, vertices):
    ...
    is_dataframe = isinstance(vertices, cudf.DataFrame) or isinstance(
        vertices, dask_cudf.DataFrame
    )
...
def bfs(
    ...
    *,
    ...
    include_vertices, exclude_vertices, target_vertices : cudf.Series or list, optional
        Concrete vertex predicate sets. Callable predicates are not accepted.
        Passing both include_vertices and exclude_vertices is invalid.

    include_edge_ids : cudf.Series or list, optional
        Concrete edge-id include set. The graph must have edge IDs.

    return_target_info : bool, optional (default=False)
        If True, return a tuple of (result, target_info), where target_info
        contains target_found and target_distance.
    ...
    if has_predicates or return_target_info:
        ...
        distances, predecessors, vertices, target_info = pylibcugraph_bfs_with_predicates(
            ...
            stop_on_first_target=target_vertices is not None,
