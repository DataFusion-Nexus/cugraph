...
def test_bfs_predicates_vertex_and_target():
    df = cudf.DataFrame({"src": [10, 10, 20, 30], "dst": [20, 30, 40, 40]})
    G = cugraph.Graph(directed=True)
    G.from_cudf_edgelist(df, source="src", destination="dst")
    result, target_info = cugraph.bfs(
        G, 10, exclude_vertices=cudf.Series([20], dtype="int32"),
        target_vertices=cudf.Series([40], dtype="int32"), return_target_info=True)
...
def test_bfs_predicates_edge_ids():
    ...
    result = cugraph.bfs(G, 0, include_edge_ids=cudf.Series([200, 400], dtype="int32"))
    distances = dict(zip(result["vertex"].to_pandas(), result["distance"].to_pandas()))
